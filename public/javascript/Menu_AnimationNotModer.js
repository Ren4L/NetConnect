...
document.querySelector(".UpDown").addEventListener("click", function(e){
    var screenHeight=document.querySelector("html").clientHeight;
    var Down=Math.round(((parseFloat(window.getComputedStyle(document.querySelector(`.flex_line`)).top)*100)/screenHeight));
    var Line_Up=parseFloat(window.getComputedStyle(document.querySelector('.UpDown')).top);
    var stepEnd=Math.round((Line_Up*100)/screenHeight);
    document.querySelector(`.flex_line`).style.top=`${stepEnd}%`;
    document.querySelector('.Down').style.top =`${stepEnd}%`
    document.querySelector(`.UpDown`).style.top=`${Down}%`;
});

document.querySelector(".Down").addEventListener("click", function(e){
    var screenHeight=document.querySelector("html").clientHeight;
    var Down=Math.round(((parseFloat(window.getComputedStyle(document.querySelector(`.UpDown`)).top)*100)/screenHeight));
    var Line_Up=parseFloat(window.getComputedStyle(document.querySelector('.flex_line')).top);
    var stepEnd=Math.round((Line_Up*100)/screenHeight);
    document.querySelector(`.flex_line`).style.top=`${Down}%`;
    document.querySelector('.Down').style.top =`${Down}%`
    document.querySelector(`.UpDown`).style.top=`${stepEnd}%`;
});
