...
router.get('/', async(req, res)=>{
  ...
  FullList.forEach(element => {
    ...
    if(req.session.userName == undefined){
        ...
        if(Acc.Photos != undefined && Acc.Photos.length != 0){
            Acc.Photos.forEach(element => {
                Photos += `<img src="${element}" class="Photos">`;
            });
        }
    ...
    .post('/', async(req, res)=>{
      ...
      FullList.forEach(element => {
        ...
        if(Acc.Photos == undefined){
            Acc.Photos = [];
        }
        Acc.Photos.push(`https://firebasestorage.googleapis.com/v0/b/netconnectv2-9fd5b.appspot.com/o/${req.files[0].fileRef.id}?alt=media&token=b6fb2b91-9a63-4634-92c6-76da6316b473`);
        FullList[index] = Acc;
        await set(ref(db, `users`), FullList);
        let Result = {result:'AvatarDone', photo:`https://firebasestorage.googleapis.com/v0/b/netconnectv2-9fd5b.appspot.com/o/${req.files[0].fileRef.id}?alt=media&token=b6fb2b91-9a63-4634-92c6-76da6316b473`};
        res.json(Result);
    });
